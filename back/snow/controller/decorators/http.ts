...
/**
 * @example
 * testMethod(@Query('id') id: string)
 */
export function Query(parameter: string): ParameterDecorator {
  return (target, key, index) => {
    Reflect.defineMetadata(META_KEYS.QUERY, { parameter, index }, target, key);
  };
}
